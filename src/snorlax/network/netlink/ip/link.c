...
static void network_netlink_ip_addr_req_addr_add(network_netlink_ip_link_req_t * req, uint16_t type, uint32_t index) {
...
extern int64_t network_netlink_ip_link_setup_req(network_netlink_t * descriptor, const char * dev, network_network_res_t callback) {
    network_netlink_ip_link_req_t * req = network_netlink_request_pop(descriptor, network_netlink_ip_link_req_t, true);
    ...
    network_netlink_ip_addr_req_addr_add(req, IFLA_LINK, req->message.ifi_index);
